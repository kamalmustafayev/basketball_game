...
/**
 * GBall class inherited from Falling object that moves the balls during the
 * game
 * 
 * @author Kamal Mustafayev
 *
 */
public class GBall extends FallingObject {
	 ...
	 *            height of the screen
	 */
	public GBall(double size, double screenWidth, double screenHeight) {
		super("ball.gif", size);
		this.size = size;
	...
	/**
	 * whole process of falling the ball
	 */
	@Override
	public void run() {
	...
	/**
	 * ball falling with increasing speed
	 */
	private void fall() {
		moveY += 0.1;
		move(moveX, moveY);
	...
	/**
	 * ball bouncing after touching the ground
	 */
	@Override
	void bounce() {
		for (int i = 0; i < 3; i++) {
